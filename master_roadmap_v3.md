...
✓ QUICKSTART 利用者向け手順 .......................... 完了(2026年6月)
   ...
   │
▷ 次        エージェント連携(read専用・MCP) ................ 計画中(2026年7月時点、着手前)
   └─ 商用 Marketing-OS 側に、KPI・意思決定ログ・整合性警告などを read-only で参照できる
      MCPサーバーを追加する計画。書き込みツールは型レベルで定義しない方針(readOnly-first)。
      OSS三本柱(SEO/広告/SNS の診断CLI・Web)とは別レイヤーで、OSS側の公開済み機能に変更はない。
```
...
4. **エージェント連携（read専用・MCP）** — 商用 Marketing-OS 側で計画中（着手前）。OAuth 2.1 + PKCE、`mcp:read` スコープのみ発行、書き込みツールは型レベルで定義しない。OSS三本柱の診断機能とは別レイヤーで、公開済みの CLI/Web に変更はない。
...
- v3.3: 2026年7月9日(「エージェント連携（read専用・MCP）」を次フェーズの計画中ノードとして追加。理由: 商用 Marketing-OS 側で MCP サーバー実装（OAuth 2.1・readOnly-first）が次の実装対象として起票されたため、着手前の段階からロードマップに記載し、進捗を追える状態にする。OSS三本柱(SEO/広告/SNS)の診断CLI・Web は npm 公開・稼働確認済みのままであり、本更新はこれらの完了状態を変更しない — 別リポの起票情報を理由に、検証済みの公開実績を「準備中」へ書き換えることはしない)
